...
asmlinkage int sys_taskreader(int pid){
	...
	int pid_process = 3477;
    struct task_struct *task;
    struct task_struct *my_parent;

    for_each_process(task){
    	if(task->pid == pid_process){
        	// state: -1 unrunnable, 0 runnable, >0 stopped
        	if(task->state == -1)
        		printk("[%d] %s, unrunnable\n", task->pid, task->comm);
        	if(task->state == 0)
        		printk("[%d] %s, runnable\n", task->pid, task->comm);
        	if(task->state > 0)
        		printk("[%d] %s, stopped\n", task->pid, task->comm);
    
        	my_parent = task->parent;
    
        	for_each_process(my_parent){
            	if(my_parent->state == -1)
            		printk("[%d] %s, unrunnable\n", my_parent->pid, my_parent->comm);
            	if(my_parent->state == 0)
            		printk("[%d] %s, runnable\n", my_parent->pid, my_parent->comm);
            	if(my_parent->state > 0)
            		printk("[%d] %s, stopped\n", my_parent->pid, my_parent->comm);
            }
